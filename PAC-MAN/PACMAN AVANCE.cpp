...
int main(){
    ...
    printf("\tBIENVENIDOS\t");
    nombre();
    printf("\nPRESIONE ""1" " = Jugar Pacman\n");
    scanf("%i",&opcion);
    printf("\n\n\n\n");
    if (opcion==1){
    pacman_completo();
   	}
   	printf("%s tuvo %i puntos",nombre,puntos1);
}
...
void pintar_mapa(){
    for(int i = 0 ; i < 78 ; i++){
        for(int j = 0 ; j < 30 ; j++){
        	gotoxy(i,j);
            if(mapa[j][i] == 'X') {setCColor(color[3]);printf("%c",205);}
            if(mapa[j][i] == '_') {setCColor(color[3]); printf("%c",250);}
            else if(mapa[j][i] == 'Y') {setCColor(color[3]);printf("%c",186);}
            else if(mapa[j][i] == 'A') {setCColor(color[3]);printf("%c",201);}
            else if(mapa[j][i] == 'B') {setCColor(color[3]);printf("%c",187);}
            else if(mapa[j][i] == 'C') {setCColor(color[3]);printf("%c",188);}
            else if(mapa[j][i] == 'D') {setCColor(color[3]);printf("%c",200);}
        }
    }
}
...
void fantasma::mover_fantasma(){
    ...
    if(fdir == 2){
        if(mapa[_y][_x+1] != 'X' && mapa[_y][_x+1] != 'A' && mapa[_y][_x+1] != 'Y' && mapa[_y][_x+1] != 'B' && mapa[_y][_x+1] != 'C' && mapa[_y][_x+1] != 'D')         
		_x++;
        else fdir = rand()%4;
    }
    if(fdir == 3){
